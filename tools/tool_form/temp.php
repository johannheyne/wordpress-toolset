		...
		*/
		...
		function __construct( $p = array( 'form_id' => '' ) ) {
			...
			// DEFAULTS {
				...
				$this->p['form_attrs']['data-form-id'] = $this->p['form_id'];

				$this->p = apply_filters( 'class/Form/param', $this->p );
		...
		private function get_field( $html, $item ) {
			...
			$html .= apply_filters( 'class/Form/before_item/form_group=' . $this->p['form_group'], $html_item, $this->p );
				...
				if ( $item['type'] === 'search'  ) {
					...
					$html .= $this->get_search_field( $item );
				}

				if ( $item['type'] === 'date'  ) {

					$html .= $this->get_date_field( $item );
				}
		...
		public function get_search_field( $p = array() ) {
		...
		public function get_date_field( $p = array() ) {
